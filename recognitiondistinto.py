    ...
    try:
        ...
        min_detection_confidence=0.7) as face_detection:
            ...
            min_tracking_confidence=0.75) as face_mesh:
                ...
                camara = cv2.VideoCapture('http://192.168.0.114:8080/?action=stream')
                while True:
                        ...
                        if video is not None:
                            ...
                            if results.detections is not None:
                                for detection in results.detections:
                                    ...
                                    #crearndo nueva ventana y dandole la rotacion a la imagen
                                    alinear = cv2.warpAffine(videorgb, m, (ancho,alto))
                                    # cv2.imshow("alinear", alinear)
                                    ...
                                    results3 = face_mesh.process(alinear)
                                        
                                    if results3.multi_face_landmarks is not None:
                                        ...
                                        for face_landmarks in results3.multi_face_landmarks:
                                            ...
                                            if y_10 >=20 and x_227 >= 20:
                                                vista_previargb = alinear[y_10-20 : y_175 +20, x_227 - 20: x_447 +20]
                                                #vista_previargb = cv2.cvtColor(vista_previargb, cv2.COLOR_BGR2RGB)
                                                altog, anchog, _ = vista_previargb.shape
                                                vista_previargb2 = cv2.resize(vista_previargb, (anchog+300,altog+300))
                                                
                                                cv2.imshow('image final', vista_previargb)
                                                results2 = face_mesh.process(vista_previargb2)
                                                
                                                print(results2.multi_face_landmarks)

                                                if results2.multi_face_landmarks is not None:
                                                    cv2.imshow('image final', vista_previargb2)
                                                    ...
                                                    for face_landmarks in results2.multi_face_landmarks:
                                                        ...
                                                        y_386 = int(face_landmarks.landmark[386].y * altog*10) 
                                                        y_374 = int(face_landmarks.landmark[374].y * altog*10) 
                                                        y_159 = int(face_landmarks.landmark[159].y * altog*10)
                                                        y_145 = int(face_landmarks.landmark[145].y * altog*10)
                                                        x_386 = int(face_landmarks.landmark[386].x * anchog*10) 
                                                        x_374 = int(face_landmarks.landmark[374].x * anchog*10) 
                                                        x_159 = int(face_landmarks.landmark[159].x * anchog*10)
                                                        x_145 = int(face_landmarks.landmark[145].x * anchog*10)

                                                        y_1 = int(face_landmarks.landmark[1].y * altog)
                                                        x_1 = int(face_landmarks.landmark[1].x * anchog)
                                                        
                                                        #image = cv2.circle(alinear, (x_386, y_386), 20, (255,255,255), 2)
                                                        # #mejilla derecha
                                                        # y_447 = int(face_landmarks.landmark[447].y * alto) 
                                                        # x_447 = int(face_landmarks.landmark[447].x * ancho) 
                                                        # #mejilla izquierda
